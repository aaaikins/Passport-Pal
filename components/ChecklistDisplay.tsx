...
export default function ChecklistDisplay({ checklist, email, onReset }: ChecklistDisplayProps) {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const checklistText = checklist.checklist.map((item) => item.text).join('\n');
      const result = await sendEmail(email, checklistText);
      if (result.success) {
        setEmailSent(true);
        ...
        <div className="flex gap-3 flex-wrap">
          ...
          <motion.button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${
              emailSent ? 'bg-green-600 text-white' : 'bg-purple-600 text-white hover:bg-purple-700'
            }`}
          >
      ...
      >
        <div className="flex items-start gap-4">
          ...
          {checklist.riskScore !== undefined && (
            <div className="text-center bg-white bg-opacity-20 rounded-lg p-4 flex-shrink-0">
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              {checklist.compliance.issues?.length > 0 && (
                <ul className="space-y-1">
                  {checklist.compliance.issues.map((issue, i) => (
                    <li key={i} className="text-sm text-gray-700">• {issue}</li>
                  ))}
                </ul>
              )}
              <span className={`mt-2 block text-sm font-medium ${getRiskScoreColor(checklist.compliance.score)}`}>
      ...
      {checklist.visaRequirements && (
        ...
        >
          <h3 className="text-lg font-semibold text-blue-900 mb-3 flex items-center gap-2">
            <Info className="w-5 h-5" /> Visa Information
          </h3>
                ...
                {item.links?.length > 0 && (
                  <div className="mt-2 space-y-1">
                    {item.links.map((link, li) => (
                      <a
                        key={li} href={link} target="_blank" rel="noopener noreferrer"
          ...
          <ul className="space-y-2">
            {checklist.recommendations.map((rec, i) => (
              <li key={i} className="text-green-800 text-sm flex items-start gap-2">
                <span className="text-green-600 font-bold">→</span> {rec}
              </li>
            ))}
          </ul>
        </motion.div>
